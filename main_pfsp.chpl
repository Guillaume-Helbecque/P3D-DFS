  ...
  proc main(args: [] string): int
  {
    ...
    select mode {
      when "sequential" {
        ...
        search_sequential(Node_PFSP, pfsp, timelimit, saveTime);
      }
      when "multicore" {
        search_multicore(Node_PFSP, pfsp, timelimit, saveTime, activeSet);
      }
      when "distributed" {
        search_distributed(Node_PFSP, pfsp, timelimit, saveTime, activeSet);
      }
      otherwise {
