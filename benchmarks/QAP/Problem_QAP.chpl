  ...
  class Problem_QAP : Problem
  {
    ...
    proc init(filename, itmax, lb, ub): void
    {
      ...
      if (ub == "heuristic") then this.initUB = GreedyAllocation(this.D, this.F, this.priority_fac, this.n, this.N);
        ...
        // NOTE: If `ub` cannot be cast into `int(32)`, an error is thrown. For now, we cannot
       ...
       while (job[j_cur] != -1) {
         ...
         for j in 0..<n {
           if !in_Z[j] {
             ...
             const cur_cost = C[idx4D(i0, j0, w, j, n)] - yw[w] - yj[j];
      ...
      ref solutions: list(string)): list(?)
    {
      ...
      if (depth == this.n) {
        ...
        local {
          ...
          const k = localLogicalQubitIndex(parent.mapping, i);
          ...
          for j0 in 0..<this.N by -1 {
            ...
            const lb_new = parent.lower_bound + incre;
            ...
            if (child.depth < this.n) {
              const lb = bound_HHB(child, best_task);
      ...
      for w_cur in 0..<n {
        ...
        while (job[j_cur] != -1) {
          ...
          for j in 0..<m {
            if !in_Z[j] {
              // reduced cost = C[w][j] - yw[w] - yj[j]
              const cur_cost = C[w*m + j] - yw[w] - yj[j];
