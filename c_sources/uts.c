 ...
 */
tree_t treeType = GEO; // Default tree type
...
void uts_parseParams(int argc, char *argv[]){
  ...
  while (i < argc && err == -1) {
    ...
    switch (argv[i][1]) {
      ...
      case 't':
        treeType = (tree_t) atoi(argv[i+1]);
        if (treeType != BIN && treeType != GEO && treeType!= HYBRID && treeType != BALANCED)
	  err = i;
        break;
      case 'a':
