...
for inst in "${tests[@]}"; do
  ...
  for ub in "${ubs[@]}"; do
    echo "======================================"
    echo "Instance=$inst LB=hhb UB=$ub (expected=$expected)"

    cmd="./main_qap.out --mode sequential --inst $inst --lb hhb --ub $ub"
...
echo "All QAP tests with HHB passed!"
